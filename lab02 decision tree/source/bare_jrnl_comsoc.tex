...
\IEEEPARstart{U}{se} the decision tree for classification based on Breast cancer dataset available at kaggle\footnote{https://www.kaggle.com/uciml/breast-cancer-wisconsin-data}. 


\section{DATA DESCRIPTION}
Breast Cancer Wisconsin (Diagnostic) Data Set is used to predict whether the cancer is benign or malignant.Features are computed from a digitized image of a fine needle aspirate (FNA) of a breast mass. They describe characteristics of the cell nuclei present in the image. \cite{street1993nuclear}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item compactness $(\text{perimeter}\ ^ 2  / \text{area} - 1.0)$ 
...
\subsection{Missing attribute values}
...
A decision tree is a decision support tool that uses a tree-like model of decisions and their possible consequences, including chance event outcomes, resource costs, and utility. It is one way to display an algorithm that only contains conditional control statements. We take the ID3(Iterative Dichotomiser 3) algorithm with information gain as a criterion for choosing features.
...
We assume $D$ is the set of samples and $a$ is the feature with $V$ possible values.

$$
\text{Gain}(D,a)=Ent(D)-\sum_{v=1}^{V}\frac{\left|  D^v \right|}{\left|  D \right|} \text{Ent}(D^v)
$$
...
\begin{figure}[h]
	...
	\includegraphics[width=0.25\textwidth,height=0.25\textwidth]{confusion_matrix}
\end{figure}


At last, we can visualize our decision tree with \texttt{graphviz} module which can generate digraph using few commands.The tree structure figure is in the appendix. 
...
\begin{figure}[h]
	...
	\includegraphics[width=0.5\textwidth,height=0.5\textwidth]{decision_tree}
\end{figure}
